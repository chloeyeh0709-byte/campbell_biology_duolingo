...
目前已匯入的課程：**Clayden 有機化學**（改編自 Clayden《Organic Chemistry》，翻譯成繁體中文），涵蓋第 1–24 章，共 24 個單元、70 堂課、123 個知識圖譜概念、210 道題目：
...
- `prisma/seed/organic-chemistry-ch20-24.json`：第 20–24 章（烯醇與烯醇化物的形成與反應、芳香環的親電取代、共軛加成與親核芳香取代、化學選擇性與保護基、區域選擇性）——同樣不需要 `matchTitle`；有 8 個概念沿用第 6、10、11、15、19 章已建立的概念名稱（羰基的親核加成、醯基衍生物的相對反應性、縮醛的形成、碳陽離子的穩定性、SN1與SN2、溴鎓離子、過氧酸環氧化、馬可尼可夫規則）

這是「同一本書陸續加新章節」的實際範例：五份 JSON 依序執行 `db:seed`，後面執行的不會刪除先前匯入的內容，而是重用既有概念、把新章節接在後面。
...
npm run db:seed -- prisma/seed/organic-chemistry-ch20-24.json
npm run dev
```
